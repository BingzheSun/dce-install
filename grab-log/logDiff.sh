...
BackupLog(){
    ...
    POD_INFO=` kubectl get pod -n $NAMESPACES -o wide `
    POD_NAME_LIST=` echo "$POD_INFO" | sed '1d' | awk '{print $1 }' `
...
DiffLog(){
    ...
    for FILENAME in ${LIST} ; do
        ...
        if [ -n "$ERROR_INFO" ] ; then
            echo "#============== find error or panic, $NAME / $NAMESPACES , report to ${BACKUP_DIR}/report_$NAME ====================="
